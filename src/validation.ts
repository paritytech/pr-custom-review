...
import { AndDistinctRule, AndRule, BasicRule, Configuration, OrRule, RuleCriteria } from "./types";
...
const ruleCriterion = ({
  ...
  return {
    ...
    ...(isMinApprovalsAllowed ? { min_approvals: Joi.number().min(1).required() } : {}),
  };
};

const ruleCriterionArraySchema = Joi.array()
  .items(Joi.object<RuleCriteria>().keys(ruleCriterion({ isMinApprovalsAllowed: true, isNameOptional: true })))
  .required();
...
const commonRuleSchema = {
  ...
  condition: Joi.alternatives([
    ...
    Joi.object().keys({ include: includeConditionSchema, exclude: excludeConditionSchema }),
  ]).required(),
...
const ruleSchema = Joi.alternatives([
  ...
  Joi.object<AndRule>().keys({ ...commonRuleSchema, all: ruleCriterionArraySchema }),
  Joi.object<OrRule>().keys({ ...commonRuleSchema, any: ruleCriterionArraySchema }),
  Joi.object<AndDistinctRule>().keys({ ...commonRuleSchema, all_distinct: ruleCriterionArraySchema }),
]);
...
export const configurationSchema = Joi.object<Configuration>().keys({
  ...
  "prevent-review-request": Joi.object<Configuration["prevent-review-request"]>()
    .keys({
      users: Joi.array().items(Joi.string()).optional().allow(null),
      teams: Joi.array().items(Joi.string()).optional().allow(null),
    })
